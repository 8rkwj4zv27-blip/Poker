# Career Mode — History Archive

Dated record of every completed Career milestone, superseded plan and old
phase writeup, oldest decisions still findable, newest first. This file is
long by design and is **not** part of the default Career reading list — read
`STATUS.md` first, and open this file only to research how something
specific was already built or decided. `STATUS.md` is the source of truth
for current state; entries here are historical and may reference systems
that later passes changed.
